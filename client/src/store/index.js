...
function GlobalStoreContextProvider(props) {
        ...
        switch (type) {
            ...
            case GlobalStoreActionType.CHANGE_LIST_NAME: {
                return setStore({
                    ...
                    listIdMarkedForDeletion: null,
                    listMarkedForDeletion: null,
                    listToPlay: store.listToPlay,
                    publishList: store.publishList,
                    commentForCard: null
                });
            }
            ...
            case GlobalStoreActionType.UPDATE_COMMENT: {
                return setStore({
                    ...
                    listIdMarkedForDeletion: null,
                    listMarkedForDeletion: null,
                    listToPlay: payload.playlist,
                    publishList: null,
                    commentForCard: payload.comment,
    ...
    store.duplicateList = async function () {
        ...
        if (response.status === 201) {
            ...
            console.log("API FAILED TO DUP LIST");
        }
    }
    ...
    store.showEditSongModal = (songIndex, songToEdit) => {
        ...
        console.log("at edit song modal")
    ...
    //COPY OF SET CURRENT LIST TO SET THE LIST BEING PLAYED
    ...
    store.updateComments = function(commentToPost){
        async function asyncUpdateComments(commentToPost, id) {
            const response = await api.getPlaylistById(id);
            if (response.data.success) {
                let playlist = response.data.playlist;
                // make username first+last name for now
                let tempUser = ""
                tempUser = auth.user.firstName + " " + auth.user.lastName
        ...
        asyncUpdateComments(commentToPost, store.listToPlay._id)
    }
    store.undo = function () {
        tps.undoTransaction();
    }
    ...
    store.canAddNewSong = function() {
        return ((store.currentList !== null) && (store.currentModal!=="EDIT_SONG") && (store.currentModal!=="REMOVE_SONG"));
    }
    store.canUndo = function() {
        return ((store.currentList !== null) && tps.hasTransactionToUndo() && (store.currentModal!=="EDIT_SONG") && (store.currentModal!=="REMOVE_SONG"));
    }
    store.canRedo = function() {
        return ((store.currentList !== null) && tps.hasTransactionToRedo() && (store.currentModal!=="EDIT_SONG") && (store.currentModal!=="REMOVE_SONG"));
    }
    store.canClose = function() {
        return ((store.currentList !== null) && (store.currentModal!=="EDIT_SONG") && (store.currentModal!=="REMOVE_SONG"));
    }
    store.canAddNewList = function() {
        return ((store.currentModal!=="DELETE_LIST") && (!store.listNameActive))
    }
